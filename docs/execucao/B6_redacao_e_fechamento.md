...
4. `results/metricas/comparacao_final.csv` e `teste_lacrado.json`
...
| Reprodutibilidade | `reproducao_bloco3.json` · `REVISAO_BLOCO3.md` · `guarda_reproducao.py` |
...
| 9 | **Variância entre sementes na CNN** medida com N sementes (ou apenas seed 42, com limitação computacional declarada). A fonte de variação que domina o braço principal é *qual subamostra de 30k caiu*, medida em `estabilidade_subamostra.json` | P6 · decisão 6 |
| 10 | **Determinismo estrito no PyTorch** (se `estrito=False` foi necessário) | B4.4 |
| 11 | **Experimento cross-attack / leave-one-attack-out** previsto como análise complementar, não executado | `README` |
| 12 | **Hardware único**: todas as medições numa máquina (Windows 10, RTX 5060 Ti). Tempos são indicativos de ordem de grandeza, não benchmark | config `tempo:` |
...
Três desfechos possíveis, e o vocabulário de cada um:
...
- por áudio: RF 11,12 ms total (predição 57,6% do custo); SVM 5,33 ms;
- em lote: o RF é ~19× melhor que o SVM;
- «não existe "o modelo mais barato" sem dizer o regime»;
- a hipótese de que o pré-processamento dominaria **não se confirmou** no ramo
  clássico — e na CNN a geração do espectrograma provavelmente domina;
...
- [ ] limitações completas (as 12 acima + as que apareceram)
...
**Atualize o `README`** com: a seção do Bloco 4, a tabela comparativa final, os
comandos novos (`gerar_espectrogramas`, `treinar_cnn`, `verificar_espectrogramas`,
`avaliar_teste_lacrado`) e a nota de que o teste foi usado uma única vez.
