...
| 23/09 | **`requirements.txt` só com pisos (`>=`)** — as versões exatas estão nos campos `ambiente` dos JSONs, mas não num arquivo instalável | não | B6.6: gravar `pip freeze > requirements-lock.txt` no ambiente que gerou os resultados |
...
| 23/09 | **Worktree antigo em `.claude/worktrees/amazing-yonath-d9913c`** (24/08), com cópia velha de `src/` e `config.yaml` | não — mas uma busca de texto na pasta encontra código desatualizado | `git worktree remove` depois de conferir que o ramo dele não tem nada que falte no `master` |
